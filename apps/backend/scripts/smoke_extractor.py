"""Manual smoke check for the text extractor.

Not a pytest module, and deliberately not named test_*. Every function here
talks to a running extractor on localhost:6004 and reports a boolean rather than
asserting, so collected as tests they would pass whatever the service did while
appearing to cover it.

    python scripts/smoke_extractor.py
"""
