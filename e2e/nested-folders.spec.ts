...
test.describe('Nested Folders', () => {
  test('should create 25 nested folders', async ({ page }) => {
    ...
    for (let i = 1; i <= 25; i++) {
      ...
      const folderCard = page.locator('.bg-white').filter({ hasText: folderName }).first();
  ...
  test('should delete all nested folders by deleting root', async ({ page }) => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      const openButton = page.getByText(folderName).locator('..').locator('..').getByRole('button', { name: /open/i });
      await openButton.click();
    ...
    const folderCard = page.locator('.bg-white').filter({ hasText: 'Nested-1' }).first();
